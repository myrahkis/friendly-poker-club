...
<template>
  <div class="tournaments-descriptions">
    ...
    <div class="description-block">
      ...
      <div class="description-block-info">
        <h4>Френдли турнир</h4>
        <p>
          Турнир, в котором игрокам выдаются карточки «Покажи карту» и «Крутить
          дважды», по одной штуке каждому, которыми игрок может воспользоваться
          один раз за турнир. На время использования карточек, таймер принятия
          решения останавливается. После использования, карточка забирается
          дилером.
        </p>
        <p>
          Карточкой <strong>«Покажи карту»</strong> - можно воспользоваться,
          когда игрок остаётся один на один с «агрессором». Предъявив данную
          карту, игрок выбирает одну из карт оппонента и просит её показать.
          Оппонент, в свою очередь, переворачивает (показывает) карту, которую
          выбрал игрок. После чего игрок принимает решение о своём дальнейшем
          действии. Возможно использовать только одну карточку за раздачу.
        </p>
        <p>
          Карточкой <strong>«Крутить дважды»</strong> можно воспользоваться
          только в олл-ине двух игроков, после шоу дауна (когда карты игроков,
          выставившихся в олл-ин раскрыты). Игрок, оценив силу своей руки и руки
          оппонента может предъявить карточку «Крутить дважды». В этом случае
          дилер выкладывает либо две полных доски, либо два тёрна, либо два
          ривера, в зависимости от того, на какой улице игроки выставились в
          олл-ин. Итоговый Банк, в случае использования карточки, делится на 2
          равных части. По каждой из них определяется победитель и каждая из
          двух частей банка делится в зависимости от комбинаций оппонентов. Если
          Игрок1 выиграл на первой доске, а на второй доске проиграл – банк
          делится пополам. Если Игрок1 выиграл на первой доске, а на второй у
          игроков «ничья», то Игрок1 получает 75% от общего банка, а Игрок2 -
          25%.
        </p>
        <p>Возможно использовать только одну карточку за раздачу.</p>
      </div>
    </div>
    <div class="description-block">
      <div class="description-block-info">
        <h4>Буст турнир</h4>
        <p>
          Вариант турнира при котором стоимость входа в этот турнир дешевле
          обычного, но и стартовый стек — короткий. Ваша задача — набить стек с
          первой попытки. Если "раскрутиться" не удалось, вы можете сделать
          повторный вход (ре- энтри) за стандартнуюстоимость получив стандартный
          стек.
        </p>
      </div>
      <div class="desc-pic">
